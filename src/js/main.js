...
var pizzaElementGenerator = function(i) {
  ...
  pizzaImageContainer.style.width="col-md-6";  // changed from 35% width to col-md-6
  ...
  // changed from 65% width to col-md-6
  pizzaDescriptionContainer.style.width="col-md-6";
...
// this function below needs optimization MM
// resizePizzas(size) is called when the slider in the "Our Pizzas" section of the website moves.
var resizePizzas = function(size) {
  window.performance.mark("mark_start_resize");   // User Timing API function

  // Changes the value for the size of the pizza above the slider
  // used getElementById instead of querySelector to better optimize
  ...
  function determineDx (elem, size) {
    ...
    // put getElementById in place of querySelector for better optimization
  ...
  function changePizzaSizes(size) {
    // Created defaultPizza variable so that we're not changing every pizza's size, just one pizza, sense all are the same size
    // changed to .getElementsByClassName instead of querySelectorAll
    var defaultPizza = document.getElementsByClassName("randomPizzaContainer")[0];
    var dx = determineDx(defaultPizza, size);
    // calculate newwidth for pizzas by using the defaultPizza
    var newwidth = (defaultPizza.offsetWidth + dx) + 'px';
...
// removed pizzaDiv out of from for loop because this only needs to happen once
...
function updatePositions() {
  frame++;
  // created scroll variable and set it equal to document.body.scrollTop that was
  // originally inside the for loop
  var scroll = (document.body.scrollTop / 1250);
  // saved array length of items into separate itemsLength variable
  // so the array's length isn't accessed each iteration
  var items = document.getElementsByClassName('mover'),
  itemsLength = items.length;
  // placed phase variable outside of the for loop
  var phase = [];
  // created two loops, one for phases 0 - 4 and the other for positions 0 - items.length
  for (var i = 0; i < 5; i++) {
    phase.push(Math.sin(scroll + i) * 100);
  }

  for (var x = 0, max = itemsLength; x < max; x++) {
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // Moved DOM call outside of the for loop and saved to local variable movingPizzas
  var movingPizzas = document.getElementById('movingPizzas1');
  // created variable to use the inner.height properties to calculate the required number of pizzas
  ...
  for (var i = 0; i < frameHeight; i++) {
    ...
    // using local variable movingPizzas that I created above the for loop
